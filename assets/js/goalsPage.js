...
async function loadSuggestions() {
    ...
    const data = await res.json();
    if (!res.ok || !data.success) return showAlert(data.message || 'Erro ao gerar sugestões');
    ...
    <div class="card mb-2">
      <div class="card-body">
        ...
        <div class="text-muted small mb-2">${escapeHtml(s.descricao)}</div>
        <div class="small text-muted mb-2">
          KPIs: ${(s.kpis || []).map(k => escapeHtml(k.label)).join(', ')}
        </div>
        <button class="btn btn-sm btn-primary" data-use-suggestion="${idx}">
          Usar sugestão
        </button>
      </div>
    </div>
